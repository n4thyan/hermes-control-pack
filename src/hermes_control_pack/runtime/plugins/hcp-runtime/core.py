...
def register(ctx) -> None:
    ...
    # Register a startup notice callback so the TUI renders the HCP status
    # line through the normal welcome UI (after plugin discovery, during
    # banner construction). This replaces the old stdout/stderr/WriteConsole
    # hack which was cleared by the TUI.
    try:
        ...
        ctx.register_startup_notice(lambda: build_startup_line(home))
    except Exception:
        pass
