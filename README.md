...
After running `hcp setup`, you can launch Hermes from **any** directory — HCP stays active. `hcp setup` is the one-time HCP installation/configuration flow (plugin enable, skills, SOUL merge, compressor selection). The `--project` argument selects the initial project used during setup, but after installation HCP automatically detects and onboards additional repositories at runtime — users do not need to run setup again for each repo. The runtime auto-detects the project (nearest Git root or cwd) and initializes `.hcp/state/` via `ProjectStateStore.ensure()` on the first turn, so a fresh repo starts with empty project state and simply accumulates continuity as work proceeds.
